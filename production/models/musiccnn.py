...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=8, elem=512):
        self.inplanes = 64
    ...
    def forward(self, inp, get_embedding=False, process_embedding=False):
        res = []
        all_series = []
        if process_embedding:
            return self.fc3(inp[0], inp[1])
        for x in inp:
            x = self.conv1(x)
            x = self.bn1(x)
            x = self.relu(x)
...
def resnet18(weights_path, **kwargs):
